...
void test03()
{
    ...
    // 为目标容器开辟空间(根据 v1 和 v2的大小开辟 _target的大小)
    _target.resize(max(v1.size(), v2.size()));
    cout << "v1 和 v2的差集为：" << endl;
    vector<int>::iterator itEnd = set_difference(v1.begin(), v1.end(), v2.begin(), v2.end(), _target.begin());
    for_each(_target.begin(), itEnd, fn);
    cout << endl;

    cout << "v2 和 v1的差集为：" << endl;
    vector<int>::iterator itEnd1 = set_difference(v2.begin(), v2.end(), v1.begin(), v1.end(), _target.begin());
    for_each(_target.begin(), itEnd1, fn);
    cout << endl;
}
int main()
{
